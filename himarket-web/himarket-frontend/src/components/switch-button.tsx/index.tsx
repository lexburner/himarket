...
const MultiSwitchButton: React.FC<MultiSwitchProps> = ({
  ...
  if (buttonRefs.current.length !== options.length) {
    buttonRefs.current = Array(options.length)
      .fill(null)
      .map((_, i) => buttonRefs.current[i] || createRef<HTMLButtonElement>().current);
  }
  ...
  useEffect(() => {
    ...
    const measureDimensions = () => {
      ...
      const dimensions = buttonRefs.current.map((ref) => {
        if (ref) {
          return { offsetLeft: ref.offsetLeft, width: ref.offsetWidth };
        }
        return { offsetLeft: 0, width: 0 }; // Default if ref is null
      });
  ...
  console.warn(buttonDimensions, 'activeButtonClassName...');
      ...
      {options.map((option, index) => (
        <button
          ...
          className={`relative cursor-pointer z-10 flex items-center justify-center rounded-lg px-6 py-1.5 text-sm font-medium transition-colors duration-300 ease-in-out whitespace-nowrap ${buttonClassName} ${
            index === activeIndex ? activeButtonClassName : inactiveButtonClassName
          }`}
          key={option.value}
